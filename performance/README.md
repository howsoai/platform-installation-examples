...
A stable network connection to the cluster is required.  A key reason that using even a powerful developer workstations can lead to issues is they frequently move, use WiFi and/or VPNs.  A large workload may expect to run for hours or days, such network interuptions can lead to the client application losing connection to the cluster, and the run failing. 
